...
Упрощённая версия: один надёжный путь открытия с перебором backend-ов,
параллельное чтение выбранных ролей с отдельным lock на камеру, проверка
разрешения и near-black кадров, дренаж буфера драйвера и один прогрев.
Сложная многоступенчатая «самодиагностика» (переоткрытие потоков в цикле,
многофазный догрев, доказательство FOURCC) убрана.
"""
...
class CameraManager:
    ...
    # ---------- совместимость: переоткрытие оставлено no-op-friendly ----

    def reopen_roles(self, roles, timeout: float | None = None) -> dict:
        """В упрощённой версии переоткрытие потоков не поддерживается.

        Раньше при просадке USB менеджер пересоздавал VideoCapture на
        лету. Теперь ошибка камеры считается фатальной для шага, как и
        описано в README (production-цикл уходит в FAULT).
        """
        requested = tuple(dict.fromkeys(roles))
