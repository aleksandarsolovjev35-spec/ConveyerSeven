"""Одиночный прогон инспекции.

Production-инспекция снимает один свежий кадр и итог каждого правила — это
его ``triggered`` по этому кадру. Многопрогонового голосования в системе
нет: ``INSPECTION_RUNS = 1``. Модуль оставлен как тонкая обёртка, которая
собирает метаданные результата в том же формате, что ожидают отчёт правой
панели и архив (``run_cards``, ``run_status``, ``vote_details`` и т.п.).
"""
...
def _one_run(items, label: str) -> object:
    """Вернуть единственный элемент набора прогонов либо ошибиться."""
    values = list(items)
