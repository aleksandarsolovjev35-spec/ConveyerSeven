"""Единичный прогон инспекции.

Вспомогательные функции этого модуля используются для построения отчёта 
(run_cards, статусы области, выбор картинки). ``INSPECTION_RUNS = 1`` 
обеспечивает, что ``run_cards`` содержит один элемент, а evidence — 
единственный прогон.
"""
