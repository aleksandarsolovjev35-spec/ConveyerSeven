    ...
    def _build_status(self) -> dict:
        ...
        for part in parts_snapshot:
            ...
            line_parts.append({
                ...
                # проходят в шаге приезда. При серии через пустую ячейку
                # лепесток остаётся открытым (DIST1>0) — следующий корпус
                # той же категории не придерживается, а ждёт сброса.
                "held": (
                    position == self.OFFSET_REJECT
                    and part.route_category != CATEGORY_GOOD
                    and not dropping
                    and int(dist.get("dist1_position", 0)) == 0
                ),
                "dropping": dropping,
            })
