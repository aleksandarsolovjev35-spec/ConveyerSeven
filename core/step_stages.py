...
# Пауза визуализации фаз шага в HMI: каждый переход фаз задерживается на это
# время, чтобы оператор видел этапы шага по отдельности. На физику линии не
# влияет: пауза берётся тогда, когда лента уже остановлена или ещё не
# тронулась. Производственное значение приходит из calibration.json
# (stage_trace_time = 0.5 с); константа ниже — только программный fallback
# для случаев, когда явное значение не передано.
