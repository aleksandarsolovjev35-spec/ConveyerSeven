...
OPTIONAL_DEFAULTS = {
    ...
    "settle_time": 0.5,
    # Пауза визуализации фаз шага в HMI: каждый переход фаз шага
    # задерживается на это время, чтобы оператор видел этапы по отдельности.
    # На физику линии не влияет: пауза берётся тогда, когда лента уже
